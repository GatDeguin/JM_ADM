...
export class PayablesTreasuryRepository {
  ...
  create(data: CreateAccountsPayableInput): Promise<AccountsPayableDto> {
    return this.prisma.accountsPayable.create({ data: data as any, include: accountsPayableInclude }) as Promise<AccountsPayableDto>;
  }

  update(id: string, data: UpdateAccountsPayableInput): Promise<AccountsPayableDto> {
    return this.prisma.accountsPayable.update({ where: { id }, data: data as any, include: accountsPayableInclude }) as Promise<AccountsPayableDto>;
  }
  ...
  runAction(id: string, payload: ActionAccountsPayableInput): Promise<AccountsPayableDto> {
    return this.prisma.$transaction(async (tx: any) => {
      ...
      return tx.accountsPayable.update({ where: { id }, data: { balance: { decrement: payload.amount as never }, status: "partial" }, include: accountsPayableInclude }) as Promise<AccountsPayableDto>;
    });
  }
}
