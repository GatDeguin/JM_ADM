...
test("e2e/alta anidada SKU + retorno al origen", async ({ page }) => {
  ...
  await mockJson(page, "**/masters/contextual/entities/sku", "POST", { id: "sku-new", label: "SKU Nest", meta: "PB · Presentación" }, 201);
  ...
  await expect(skuSection.getByRole("option", { name: "SKU Nest" })).toHaveAttribute("aria-selected", "true");
});

test("e2e/retorno origen autoselecciona entidades contextuales", async ({ page }) => {
  await mockJson(page, "**/masters/contextual/entities/producto/options", "GET", [{ id: "pb-ctx", label: "Producto contextual" }]);
  await mockJson(page, "**/masters/contextual/entities/formula_version/options", "GET", [{ id: "fv-ctx", label: "F-1 v1" }]);

  let payload: any = null;
  await page.route("**/production/production-orders", async (route) => {
    if (route.request().method() !== "POST") return route.fallback();
    payload = route.request().postDataJSON();
    return route.fulfill({ status: 201, contentType: "application/json", body: JSON.stringify({ id: "op-ctx", code: "OP-CTX" }) });
  });

  await page.goto("/operacion/produccion/nueva?contextualEntityType=producto&contextualEntityId=pb-ctx");
  await page.locator("section", { hasText: "Versión de fórmula" }).getByRole("option", { name: "F-1 v1" }).click();
